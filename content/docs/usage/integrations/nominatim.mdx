...
[Barrelman](barrelman) outranks Nominatim for geocoding, so when both are configured Nominatim only serves coordinates Barrelman has no data for — which keeps you well inside the public instance's rate limit.
