...
Some user integrations are **end-to-end encrypted**: your credentials are encrypted on your device before upload, and the Parchment server can't read them. These are the right choice for integrations that touch personal data like location history.
...
| **Geocoding** | Address-to-coordinate lookup and reverse geocoding |
...
When a connected integration's own service starts failing (for example, a self-hosted location history server goes down), Parchment doesn't show error popups. Instead the integration is flagged as **Not responding**: a warning badge appears on its card in **Settings → Integrations** and a dot marks the Integrations entry in the settings navigation. The flag clears automatically once a request succeeds, a connection test passes, or the configuration is saved again.
...
|---|---|
| [Barrelman](integrations/barrelman) | Search, Autocomplete, Search Category, Geocoding, Place Info, Spatial Contains, Spatial Children |
...
| [OpenStreetMap](integrations/openstreetmap) | *(configuration only — stores OAuth credentials for user authentication)* |
...
| Integration | Capabilities | Encryption | Requires |
|---|---|---|---|
| [OpenStreetMap Account](integrations/openstreetmap-account) | Edit Map | Server-stored | OpenStreetMap (system) |
| [Dawarich](integrations/dawarich) | *(plumbing only — no capabilities yet)* | End-to-end | Identity setup |

**End-to-end** integrations require identity setup (Settings → Encryption keys) so your device has a key to encrypt with. **Server-stored** integrations work without it, since the server holds the key.
