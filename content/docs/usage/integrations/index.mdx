...
User integrations come in two encryption flavors:

- **Server-stored** — the server encrypts credentials at rest under its master key. Necessary for integrations where Parchment must call the third party without you present (e.g. scheduled fetches, OAuth refresh flows).
- **End-to-end** — credentials are encrypted on your device under your personal key before upload. The Parchment server stores only opaque ciphertext. Used for integrations that touch sensitive personal data (location history, private self-hosted URLs) and don't need the server to act on a schedule.
